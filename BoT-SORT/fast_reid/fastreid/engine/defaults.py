    ...
    def test(cls, cfg, model):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            ...
            try:
                ...
                # Ensure valid result format
                if not isinstance(results_i, OrderedDict):
                    logger.warning(f"Test results for {dataset_name} are not in the expected format. Using empty results.")
                    results_i = OrderedDict()

            except Exception as e:
                logger.error(f"Exception during evaluation on dataset {dataset_name}: {e}")
                results_i = OrderedDict()  # Default to empty OrderedDict on error

            results[dataset_name] = results_i

            if comm.is_main_process():
                if not isinstance(results_i, dict):
                    logger.error(f"Invalid evaluation results format: {results_i}")
                    results_i = OrderedDict()  # Ensure it remains a dict
                ...
                try:
                    ...
                    logger.error(f"Error printing CSV format for dataset {dataset_name}: {e}")
