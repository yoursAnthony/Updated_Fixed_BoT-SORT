...
def main(args):
    ...
    if args.mot in ['17', '20']:
        ...
        save_path = os.path.join(args.save_path, str(args.mot) + '-ReID')
        os.makedirs(save_path, exist_ok=True)
        ...
        # Get gt data
        data_path = os.path.join(args.data_path, str(args.mot), 'train')
            ...
            for d in range(np.size(det, 0)):
                ...
                if f < num_frames // 2:
                    ...
                    try:
                        ...
                        print(f"Error saving {fileName}: {e}")
                        print(patch.shape)
                        # # Display the patch using OpenCV
                        # cv2.imshow("Error Patch", patch)
